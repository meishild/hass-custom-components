const locale = {
  'zh-Hans':{
    ...
    uvLevels:[
      '可安全户外运动哦', '涂擦SPF15的防晒霜',
      '涂擦SPF30的防晒霜','涂擦SPF30以上的防晒霜，遮阳并躲在阴凉处' ,
      '紫外线过于强烈，请勿外出'
    ]
  },
  da: {
          ...
          <div style="align-items: baseline;">
            <div style="align-items: center;">
              ...
              <template is="dom-if" if="[[weatherObj.attributes.aqi]]">
                <div class$ = "aqi [[aqiLevel(weatherObj.attributes.aqi.aqi)]]">AQI [[roundNumber(weatherObj.attributes.aqi.aqi)]]</div>
              </template>
              <template is="dom-if" if="[[weatherObj.attributes.uv]]">
                <div class$ = "uv [[uvLevel(weatherObj.attributes.uv.level)]]">UV [[roundNumber(weatherObj.attributes.uv.uv)]]</div>
              </template>
            </div>
          ...
          },
          {
            ...
            ticks: {
              ...
              callback: function(value, index, values) {
                var data = value.toLocaleDateString(locale, { weekday: 'short' });
                var time = value.toLocaleTimeString(locale, { hour: 'numeric' });
                return mode === 'hourly' ? time : data;
              },
            },
          }],
          yAxes: [{
...
customElements.define('hf-weather-card', WeatherCardChart);
